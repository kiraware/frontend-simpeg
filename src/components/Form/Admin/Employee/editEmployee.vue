                ...
                <form @submit.prevent="handleSubmit">
                    <div class="divide-y divide-white space-y-4 px-4">
                        ...
                        <div class="relative w-[40rem] flex ">
                            <input type="text" id="nipm" v-model="nipm" required
                                class="block px-2.5 pb-2 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-400 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " />
                            <label for="nipm"
                                ...
                                Induk Pegawai Muhammadiyah</label>
                        </div>
                        <div class="relative">
                            <select @click="fetchSchools" v-model="unit_kerja_id" required
                                ...
                                <option v-for="school in schools" :key="school.id" :value="school.id">{{ school.nama
                                    }}</option>
                            </select>
    ...
    setup(props, { emit }) {
        ...
        const fetchSchools = async () => {
            try {
                ...
                schools.value = response.data.schools.map(school => ({
                    id: school.id,
                    nama: school.nama,
                }));
                const selectedSchool = schools.value.find(school => school.nama === props.userData.unit_kerja.nama);
                if (selectedSchool) {
                    unit_kerja_id.value = selectedSchool.id;
                }
            } catch (error) {
        ...
        onMounted(async () => {
            fetchSchools();
            nama.value = props.userData.nama;
            nipm.value = props.userData.nipm;
            status_kepegawaian.value = props.userData.status_kepegawaian;
        });
        ...
        return { nama, nipm, unit_kerja_id, status_kepegawaian, schools, loading, errorMessage, handleSubmit, closeModal };
    },
}
</script>
